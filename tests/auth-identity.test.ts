...
describe("principal participant identity", () => {
  it("adds a stable participant id to host principals", async () => {
    ...
    expect(principal).toEqual({ kind: "host", participantId: "host" });
    expect(principal ? principalParticipantId(principal) : null).toBe("host");
  });

  it("issues and verifies a server-generated guest participant id", async () => {
    const issued = await issueGuestSessionCookie("s1", "Alice");

    expect(issued.participantId).toMatch(/^guest_[0-9a-f-]+$/);
    ...
    expect(principal ? principalParticipantId(principal) : null).toBe(
