...
- #184 Finalize flow departed-participant matching by participant ID: use stable participant identities instead of display names so duplicate names do not mislabel pending tracks while finalize waits or times out.
...
- #173 Auth session lifecycle: treat this as the umbrella sequence for surviving mid-flow expiry without replacing the interim stateless JWT model, then land `#174` through `#177` as its scoped follow-ups.
- #174 Sliding renewal for host and guest cookies: start this first inside `#173`, because renewal is the primary fix that keeps active sessions alive during normal use.
- #175 Centralized client 401 handling via `apiFetch`: do this after `#174`, because the fallback redirect and studio signal should sit on top of the renewed-cookie path rather than papering over avoidable expiry.
- #176 Studio auth-expired banner and re-auth recovery: land this after `#175`, because the studio-specific banner depends on the shared `apiFetch` 401 policy and callback hook.
- #177 Recording-state fallback and LiveKit token TTL alignment: this can run alongside the `#173` chain after `#174`, because it shares the same expiry problem space but hardens the recording control plane directly instead of depending on the studio banner UX.
...
- #180 Cross-visit exit-guard behavior for recoverable backups: decide whether leftover local backups should arm the studio exit guard after a reload, and how to distinguish genuinely recoverable audio from already-uploaded-but-uncleaned manifests.
